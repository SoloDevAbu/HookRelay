...
export const findEndpointById = async (
  id: string,
...
export async function createEndpoint(data: NewEndpoint): Promise<Endpoint> {
  const result = await db.insert(endpoints).values(data).returning();
...
export const findEndpointsByTenantId = async (
  tenantId: string,
): Promise<Endpoint[]> => {
  const result = await db
    .select()
...
export const findEndpointsByTenantIdAndEventType = async (input: {
  tenantId: string;
  eventType: string;
}): Promise<Endpoint[]> => {
  const result = await db
    ...
    .where(
      and(
        eq(endpoints.tenantId, input.tenantId),
        eq(endpoints.status, "active"),
        or(
          isNull(endpoints.eventTypeFilter),
          sql`${endpoints.eventTypeFilter} @> ${JSON.stringify([input.eventType])}::jsonb`,
        ),
      ),
    );
...
export const updateEndpointStats = async (
  endpointId: string,
  stats: Partial<Endpoint>,
): Promise<void> => {
  await db
    .update(endpoints)
    .set({
